  ...
  stages {
    ...
    stage('Release docker image for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          set +e
          docker images | grep review-middleware | grep $feature_name
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          set +e
          docker images | grep review-middleware | grep latest
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          sed -i "s/review-middleware:latest/review-middleware:$feature_name/g" cmd/review-middleware/k8s/02-review-middleware.yaml
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh 'sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/review-middleware/k8s/02-review-middleware.yaml'
